...
> Baseline: JDK[^jdk] 21 `jlink`/`jpackage`, JavaFX 21 where applicable, and native builds for each supported OS[^os]/architecture. Reviewed: 2026-09-24.
...
|-------|----------|-------|
| Maven package | Application JARs[^jar]/resources | Main class, dependency closure, reproducibility |
...
Build native packages on their target platform: Windows, macOS, and Linux packages have different tool prerequisites and formats. An x64 build does not establish ARM[^arm] compatibility. See [packaging overview](https://docs.oracle.com/en/java/javase/21/jpackage/packaging-overview.html).

## Desktop-specific failures

- Resolve bundled resources through classpath/module APIs[^api]. Store mutable configuration, databases, caches, and logs in appropriate user/application data directories, not beside a protected executable.
- FXML[^fxml] reflection may require `opens ... to javafx.fxml` in a modular application. Test the packaged runtime, not only Maven's development launcher.
- Verify native libraries for SQLite, GIS[^gis]/image codecs, and other integrations on each target. A successful compile does not validate DLL[^dll]/shared-library loading.
...
Exercise first launch, paths with spaces/non-ASCII[^ascii] characters, a standard user account, offline startup, configuration persistence, upgrade, uninstall, and a corrupted/locked data file. Verify that uninstall preserves user data according to policy.

Sign/notarize artifacts using the target platform's distribution requirements and verify the final downloaded artifact. Bundle patched runtimes in subsequent releases: users updating their system JDK do not patch your embedded runtime. Publish checksums, SBOM[^sbom]/provenance, supported platforms, and recovery instructions with the release.
...
[^os]: Operating System.
[^jar]: Java Archive.
[^arm]: Originally Acorn RISC Machine, later Advanced RISC Machines; RISC means Reduced Instruction Set Computer. Arm now names a processor architecture family.
