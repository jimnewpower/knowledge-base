...
A connection pool limits concurrent database work. Treat acquisition delay, SQL execution time, and transaction duration as separate measurements.

Related: [transactions](transactions-and-isolation.md), [JPA and Hibernate](jpa-and-hibernate.md), [SQL tuning](sql-query-tuning.md), [SQLite](sqlite.md).
...
Method fragment; imports `java.sql.SQLException`, `java.util.Optional`, and `javax.sql.DataSource`. Assumes `customer(id, name)` with a unique, non-null ID and non-null name. The caller owns the data source; this method owns its checkout.
...
For manual JDBC transactions: acquire, disable auto-commit, execute, commit; on failure, roll back and preserve the original exception if rollback also fails. Close in every path. Do not hold a checkout while waiting for an HTTP call, user interaction, or lengthy computation.
...
4. Check database CPU, I/O, and connection budget before increasing pool size.
