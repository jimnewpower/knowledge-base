...
> Baseline: Windows 11 / Windows Server with PowerShell 7; Windows networking cmdlets require the corresponding OS[^os] modules. Reviewed: 2026-09-25.
...
`CPU`[^cpu] is accumulated processor time, not instantaneous utilization. Find a listening connection's `OwningProcess`, then inspect that PID[^pid]. An empty result or access error needs interpretation; it does not by itself prove a service is healthy or absent.
...
| SHA[^sha]-256 of a file | `Get-FileHash -LiteralPath $path -Algorithm SHA256`[^sha256] |
...
- [Windows TCP connection inspection](https://learn.microsoft.com/en-us/powershell/module/nettcpip/get-nettcpconnection)[^tcp]
