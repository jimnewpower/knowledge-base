...
> Baseline: Jakarta Faces 4.1 and PrimeFaces 15 using its Jakarta artifact. Legacy JSF 2.2 / PrimeFaces 13 applications need their own namespace and API checks. Reviewed: 2026-09-24.

Debug Faces as a server-side component lifecycle. An AJAX request executes part of that lifecycle and renders selected component subtrees.

Related: [Java and Jakarta modernization](java-jakarta-modernization.md), [Spring Security](spring-security.md), [JPA and Hibernate](jpa-and-hibernate.md).
...
Facelet fragment. Requires a CDI bean named `profileView` with `name` getters/setters and a `save()` action. Business validation and authorization belong in its service boundary too.
...
Keep the AJAX target wrapper rendered even when its contents are conditional: the browser needs an existing element to replace. Naming containers affect client IDs; use an absolute expression such as `:profile:messages` when crossing them. Do not nest HTML forms. See [processing](https://github.com/primefaces/primefaces/blob/master/docs/15_0_0/core/ajaxProcessing.md) and [rendering](https://github.com/primefaces/primefaces/blob/master/docs/15_0_0/core/ajaxRendering.md).
...
- Keep view state small: IDs and editable DTOs, not a persistent collection or open Hibernate session. Getters may run repeatedly; avoid database work and side effects in them.
...
Inspect the submitted component IDs and view state, then conversion/validation messages, model values, action execution, and returned partial-response targets. A successful HTTP status alone does not prove that the action ran.
