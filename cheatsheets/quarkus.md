...
> Baseline: Quarkus 3.x, Jakarta APIs, Java 21, Maven; select an exact supported platform release for the project. Reviewed: 2026-09-25.

Quarkus performs substantial work at build time. Treat the platform BOM, extensions, build configuration, and packaging mode as a coordinated choice.

Related: [Spring Boot](spring-boot.md), [Maven](maven.md), [JDBC and HikariCP](jdbc-hikaricp.md), [testing](testing.md).
...
The final command assumes default fast-JAR packaging. Deploy the entire `quarkus-app` directory, not just `quarkus-run.jar`. Dev mode is for local development; Dev Services can provision infrastructure when the necessary container runtime is available.

## Framework boundaries

| Concern | Quarkus choice | Check |
|---------|----------------|-------|
| DI | ArC/CDI, `@ApplicationScoped`, constructor injection | Bean discovery and build-time removal of unused beans |
| REST | Jakarta REST with Quarkus REST extensions | Match extensions to the platform BOM; do not mix REST stacks casually |
...
A native executable can reduce startup time and memory in some workloads, but increases build/toolchain constraints. Test reflection, resource loading, serialization, TLS, and native dependencies in the actual executable. Measure a representative JVM baseline first.

Do not run blocking JDBC, file I/O, or long model calculations on an event-loop thread. Quarkus REST dispatch depends on method signature and annotations; choose a documented blocking/worker or virtual-thread path for the selected release. A reactive return type alone cannot make a blocking library nonblocking.
