...
> Baseline: Java 21 project context; Maven coordinates omit versions intentionally. Select releases compatible with the deployed JDK and framework. Reviewed: 2026-09-25.
...
These APIs ship with Java 21 and need no third-party Maven dependency. See the [Java 21 API reference](https://docs.oracle.com/en/java/javase/21/docs/api/index.html).
...
| HTTP calls | `java.net.http.HttpClient` | A starting point for HTTP integrations; define timeouts and response limits; see [HTTP clients](http-clients-webhooks.md) |
...
| Relational database access | `java.sql`, `javax.sql` | JDBC interfaces ship with the JDK; database drivers and pools are separate dependencies |
...
[Apache Commons](https://commons.apache.org/) is a collection of independently released components. Add only the components needed; their Maven group IDs are not all the same.

| Library / official reference | Maven dependency | Use when / decision boundary |
|------------------------------|------------------|------------------------------|
| [Jackson](https://github.com/FasterXML/jackson) | 2.x: `com.fasterxml.jackson.core:jackson-databind`; 3.x: `tools.jackson.core:jackson-databind` | JSON binding, trees, and streaming. Follow the application's Jackson major version and module set; see [Jackson](jackson-json.md) |
| [Apache Commons CSV](https://commons.apache.org/proper/commons-csv/) | `org.apache.commons:commons-csv` | Read and write delimited records with quoting and dialect rules; see [CSV and Excel](csv-excel-poi.md) |
| [Apache POI](https://poi.apache.org/components/) | `org.apache.poi:poi-ooxml` for OOXML, including `.xlsx`; `org.apache.poi:poi` for core/legacy `.xls` | Work with Office file structures. Choose a streaming approach for large workbooks; see [CSV and Excel](csv-excel-poi.md) |
...
| [Apache Commons IO](https://commons.apache.org/proper/commons-io/) | `commons-io:commons-io` | Stream, file, and directory utilities beyond `java.nio.file`. Keep resource ownership and memory limits explicit |
...
| Library / official reference | Maven dependency | Use when / decision boundary |
|------------------------------|------------------|------------------------------|
| [Hibernate ORM](https://hibernate.org/orm/documentation/) | Modern lines: `org.hibernate.orm:hibernate-core` | JPA entity persistence and a unit of work. Match the ORM, Jakarta Persistence, and framework versions; legacy coordinates/namespaces differ; see [JPA and Hibernate](jpa-and-hibernate.md) |
| [HikariCP](https://github.com/brettwooldridge/HikariCP) | `com.zaxxer:HikariCP` | Pool JDBC connections. A pool does not provide the driver or define transaction boundaries; see [JDBC and HikariCP](jdbc-hikaricp.md) |
...
| Library / official reference | Maven dependency | Use when / decision boundary |
|------------------------------|------------------|------------------------------|
| [OpenJFX](https://openjfx.io/openjfx-docs/) | `org.openjfx:javafx-controls`; `org.openjfx:javafx-fxml` when using FXML | Desktop UI. JavaFX is separate from the JDK; match JavaFX/JDK versions and package platform dependencies; see [JavaFX](javafx.md) |
| [GeoTools](https://docs.geotools.org/latest/userguide/) | `org.geotools:gt-main`, plus required format/referencing modules | GIS data access, CRS handling, and spatial processing. Keep modules on one release and follow its repository instructions; see [geospatial correctness](geospatial-correctness.md) and [rasters](raster-gis.md) |
| [JTS](https://locationtech.github.io/jts/) | `org.locationtech.jts:jts-core` | Geometry predicates, topology, buffers, and overlays. Coordinate units and projection remain the caller's responsibility; see [geospatial correctness](geospatial-correctness.md) |
...
2. Select a compatible release line, then manage explicit versions centrally or through the framework BOM. A BOM manages versions; it does not add dependencies.
...
4. Validate the packaged runtime as well as the IDE: JavaFX modules, JDBC native libraries, GIS data access, and service discovery can behave differently after packaging.
