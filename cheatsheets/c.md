...
> Baseline: ISO C17; GCC/Clang diagnostic examples on a supported Linux target. Reviewed: 2026-09-25.
...
`sizeof(array)` works where the actual array type is visible; `sizeof(pointer)` returns the pointer size. Strings require space for a terminating NUL. Binary buffers need an explicit length and may contain zero bytes.
...
- Check API-specific return values. Read `errno` only when the failing API documents its use; success does not reset it.
