# GitLab CI/CD for Maven cheat sheet

> Baseline: GitLab CI YAML with a configured Linux shell runner using JDK 21; exact server/runner features depend on the deployed version. Reviewed: 2026-09-25.
...
The workflow avoids a duplicate push pipeline for a branch with an open merge request while allowing its merge-request pipeline. Adjust explicitly for schedules, API-triggered jobs, and child pipelines. This example does not itself define deployment or release permissions.
...
Use GitLab CI Lint for the exact server version and included configuration, then exercise branch, merge-request, tag, and deliberate failure paths. Promote the verified artifact digest with its SBOM and reports.
